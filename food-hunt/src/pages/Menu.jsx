...
const Menu = () => {
    ...
    const getFoods = async () => {
        try {
            const res = await axios.get(`http://localhost:8001/api/v1/food/getAllFoods?category=${value}`)
            if (res.data.success) {
                setLoading(false)
                            ...
                            {food?.map((curElem) => (
                                <AnimatePresence mode="wait">
                                    ...
                                    <motion.div initial={{ y: 10, opacity: 0 }}
                                        ...
                                        exit={{ y: -10, opacity: 0 }}
                                        transition={{ duration: 0.4 }} className="food-card bg-red-500/10  rounded-xl flex flex-col cursor-pointer items-center p-5">
